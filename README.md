...
3. `MR_ROUTE` - the name of the Model Registry route. For now, use `odh-model-registries-modelregistry-public-rest`.  We will make this a comma separated list of route entries.  Each entry can either be of the form `namespace:name` or just `name` (in which case all namespaces will be checked and the first match used), to allow for multiple registries.  Also, the RHOAI model registry team's removal of Service Mesh as a dependency has removed the use of `istio-system` as the default namespace that prior versions of the bridge.
